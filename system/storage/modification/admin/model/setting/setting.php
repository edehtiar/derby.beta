...
class ModelSettingSetting extends Model {
                ...
                public function updateSettingTP($group, $data, $store_id = 0) {
                    $arr_keys = array('transfer_plus_license');
                    ...
                    if (count($arr_keys) > 0) {
                        $this->db->query("DELETE FROM " . DB_PREFIX . "setting WHERE store_id = '" . (int)$store_id . "' AND `" . (version_compare(VERSION, '2.0.1') < 0 ? 'group' : 'code') . "` = '" . $this->db->escape($group) . "' AND `key` NOT IN ('" . implode("','",$arr_keys) . "') ");
                    }
                    else {
