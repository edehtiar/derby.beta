    ...
    <div class="simplecheckout-block-content">
        <?php if (!empty($payment_methods)) { ?>
            <?php if ($display_type == 2 ) { ?>
                ...
                <select data-onchange="reloadAll" name="payment_method">
                    <?php foreach ($payment_methods as $payment_method) { ?>
                        <option value="<?php echo $payment_method['code']; ?>" <?php echo !empty($payment_method['dummy']) ? 'disabled="disabled"' : '' ?> <?php echo !empty($payment_method['dummy']) ? 'data-dummy="true"' : '' ?> <?php if ($payment_method['code'] == $code) { ?>selected="selected"<?php } ?>><?php 
                ...
                if (isset($payment_method['error']) and $payment_method['error'] == true) {
            ...
            <?php } else { ?>
                ...
                <?php if ((isset($payment_method['error']) and empty($payment_method['error'])) or !isset($payment_method['error'])) { /* for Simple 4.x */ ?>
             ...
             ?>
                    ...
                    <?php
                            ...
                            <div class="radio">
								<label for="<?php echo $payment_method['code']; ?>">
                                    <input type="radio" name="payment_method" value="<?php echo $payment_method['code']; ?>" disabled />
									<?php echo $payment_method['title']; ?>
								</label>
							</div>

							<?php if (isset($payment_method['error']) and !is_bool($payment_method['error'])) { ?>
								<div class="form-group"><?php echo $payment_method['error']; ?></div>
							<?php } ?>

							<?php if (isset($payment_method['error']) and $payment_method['error'] == true) {
								if (isset($payment_method['warning'])) {
									echo '<div class="form-group">'.$payment_method['warning'].'</div>';
								}
							} ?>
                    <?php
                        }
