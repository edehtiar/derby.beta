...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		$data['text_comment'] = $this->language->get('text_comment');
				...
				$storeset_yamap = $this->config->get('storeset_yamap');
                
               if(empty($storeset_yamap[$this->session->data['language']])) {
					$data['storeset_yamaps'] = false;
				} else if (isset($storeset_yamap[$this->session->data['language']])) {
					$data['storeset_yamaps'] = html_entity_decode($storeset_yamap[$this->session->data['language']], ENT_QUOTES, 'UTF-8');
				}  
				
				$storeset_yamap_text = $this->config->get('storeset_yamap_text');
				
				if(empty($storeset_yamap_text[$this->session->data['language']])) {
					$data['storeset_yamap_texts'] = false;
				} else if (isset($storeset_yamap_text[$this->session->data['language']])) {
					$data['storeset_yamap_texts'] = html_entity_decode($storeset_yamap_text[$this->session->data['language']], ENT_QUOTES, 'UTF-8');
				}  
