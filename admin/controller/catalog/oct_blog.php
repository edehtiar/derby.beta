<?php
class ControllerCatalogOctBlog extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/oct_blog');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_oct_blog->addBlog($this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/oct_blog', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_oct_blog->editBlog($this->request->get['blog_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $blog_id) {
				$this->model_catalog_oct_blog->deleteBlog($blog_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/oct_blog', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('catalog/oct_blog/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/oct_blog/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['blogs'][] = array(
				...
				'edit'        => $this->url->link('catalog/oct_blog/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $result['blog_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_title'] = $this->url->link('catalog/oct_blog', 'token=' . $this->session->data['token'] . '&sort=id.title' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/oct_blog', 'token=' . $this->session->data['token'] . '&sort=i.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/oct_blog', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/oct_blog_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['blog_id'])) {
			$data['action'] = $this->url->link('catalog/oct_blog/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/oct_blog/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $this->request->get['blog_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/oct_blog', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['blog_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$blog_info = $this->model_catalog_oct_blog->getBlog($this->request->get['blog_id']);
		}
		...
		if (isset($this->request->post['blog_description'])) {
			...
			$data['blog_description'] = $this->model_catalog_oct_blog->getBlogDescriptions($this->request->get['blog_id']);
		} else {
		...
		if (isset($this->request->post['blog_store'])) {
			...
			$data['blog_store'] = $this->model_catalog_oct_blog->getBlogStores($this->request->get['blog_id']);
		} else {
		...
		if (isset($this->request->post['blog_product'])) {
			...
			$products = $this->model_catalog_oct_blog->getBlogProduct($this->request->get['blog_id']);
		} else {
			$products = array();
		}
		...
		if (isset($this->request->post['blog_layout'])) {
			...
			$data['blog_layout'] = $this->model_catalog_oct_blog->getBlogLayouts($this->request->get['blog_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('catalog/oct_blog_form.tpl', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'catalog/oct_blog')) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $blog_id) {
			...
			$store_total = $this->model_catalog_oct_blog->getTotalStoresByBlogId($blog_id);
