...
class ControllerCatalogProductTabs extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_product_tabs->addProductTab($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_product_tabs->editProductTab($this->request->get['extra_tab_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $extra_tab_id) {
				$this->model_catalog_product_tabs->deleteProductTab($extra_tab_id);
			}
	...
	protected function getList() {
		...
		$product_tabs_total = $this->model_catalog_product_tabs->getTotalProductTabs();

		$results = $this->model_catalog_product_tabs->getProductTabs($filter_data);
		...
		foreach ($results as $result) {
			$data['product_tabs'][] = array(
				...
				'edit'        => $this->url->link('catalog/product_tabs/edit', 'token=' . $this->session->data['token'] . '&extra_tab_id=' . $result['extra_tab_id'] . $url, 'SSL')
			);
		}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['extra_tab_id'])) {
			...
			$data['action'] = $this->url->link('catalog/product_tabs/edit', 'token=' . $this->session->data['token'] . '&extra_tab_id=' . $this->request->get['extra_tab_id'] . $url, 'SSL');
		}
		...
		if (isset($this->request->get['extra_tab_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$product_tabs_info = $this->model_catalog_product_tabs->getProductTab($this->request->get['extra_tab_id']);
		}
		...
		if (isset($this->request->post['product_tabs_description'])) {
			...
			$data['product_tabs_description'] = $this->model_catalog_product_tabs->getProductTabsDescriptions($this->request->get['extra_tab_id']);
		} else {
		...
		} elseif (!empty($product_tabs_info)) {
			$data['status'] = $product_tabs_info['status'];
		} else {
			$data['status'] = true;
		}
		...
		if (isset($this->request->post['sort_order'])) {
			...
			$data['sort_order'] = $product_tabs_info['sort_order'];
		} else {
			$data['sort_order'] = '';
		}
