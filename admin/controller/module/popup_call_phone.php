...
class ControllerModulePopupCallPhone extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('popup_call_phone', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/popup_call_phone', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('module/popup_call_phone', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['popup_call_phone_data'])) {
			$data['popup_call_phone_data'] = $this->request->post['popup_call_phone_data'];
		} else {
		...
		$this->response->setOutput($this->load->view('module/popup_call_phone.tpl', $data));
	}
    ...
    $results = $this->model_module_popup_call_phone->getCallArray($filter_data);
    ...
    $history_total = $this->model_module_popup_call_phone->getTotalCallArray();

    $pagination = new Pagination();
    $pagination->total = $history_total;
    $pagination->page = $page;
    $pagination->limit = 20;
    $pagination->url = $this->url->link('module/popup_call_phone/history', 'token='.$this->session->data['token'].'&page={page}', 'SSL');
    ...
    $this->response->setOutput($this->load->view('module/popup_call_phone_history.tpl', $data));
  }
    ...
    $info = $this->model_module_popup_call_phone->getCall((int)$this->request->get['delete']);

    if ($info) {
      $this->model_module_popup_call_phone->deleteCall((int)$this->request->get['delete']);
    }
  ...
  public function delete_all() {
    ...
    $this->model_module_popup_call_phone->deleteAllCallArray();
  ...
  public function delete_all_selected() {
    ...
    if (isset($this->request->request['selected'])) {
      foreach ($this->request->request['selected'] as $request_id) {
        $info = $this->model_module_popup_call_phone->getCall((int)$request_id);

        if ($info) {
          $this->model_module_popup_call_phone->deleteCall((int)$request_id);
        }
      }
    }
    ...
    $this->model_user_user_group->addPermission($this->user->getId(), 'access', 'module/popup_call_phone');
    $this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'module/popup_call_phone');

    $this->model_module_popup_call_phone->makeDB();

    $this->model_setting_setting->editSetting('popup_call_phone', array(
        	'popup_call_phone_data' => array(
        	'status' 												=> '1',
        	'notify_status' 								=> '1',
    ...
    if (!in_array('popup_call_phone', $this->model_extension_extension->getInstalled('module'))) {
      $this->model_extension_extension->install('module', 'popup_call_phone');
    }
    
    $this->session->data['success'] = $this->language->get('text_success_install');

    $this->response->redirect($this->url->link('module/popup_call_phone', 'token=' . $this->session->data['token'], 'SSL'));
  }
  ...
  public function uninstall() {
    ...
    $this->model_module_popup_call_phone->deleteDB();
    $this->model_extension_extension->uninstall('module', 'popup_call_phone');
    $this->model_setting_setting->deleteSetting('popup_call_phone_data');
  }

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/popup_call_phone')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		foreach ($this->request->post['popup_call_phone_data'] as $key => $field) {
