<div class="modal-header">
  ...
  <h4 class="modal-title"><?php echo $text_edit_value; ?> "<?php echo $option_name; ?>"</h4>
</div>
<div class="modal-body">
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-option-value" class="form-horizontal">
    ...
    <table id="option-value" class="table table-striped table-bordered table-hover">
      ...
      <tfoot>
        <tr>
          <td colspan="3"></td>
          <td class="text-left"><button type="button" onclick="addOptionValue();" class="btn btn-primary"><i class="fa fa-plus-circle"></i></button></td>
        </tr>
      </tfoot>
    </table>
  </form>
...
<div class="modal-footer">
  <div class="text-center"><a data-form="form-option-value" class="button-save btn btn-info btn-lg"><?php echo $button_save; ?></a></div>
</div>
...
$(".button-save").on('click', function(){
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: $('#' + $(this).data('form') + ' input[type=\'text\'], #' + $(this).data('form') + ' input[type=\'hidden\'], #' + $(this).data('form') + ' select'),
		success: function(json) {
			if (json['error']) {
				$('#form-option-value').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '</div>');
				
				$('.alert-danger').delay(1500).fadeOut(500);
			}
			
			if (json['success']) {
				$('#form-option-value').before('<div class="alert alert-success"><i class="fa fa-exclamation-circle"></i> ' + json['success'] + '</div>');			
...
function addOptionValue() {
	...
	html += '  <td class="text-left"><button type="button" onclick="$(\'#option-value-row' + option_value_row + '\').empty();" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
	html += '</tr>';	
