  ...
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-mass-product-meta-h1" class="form-horizontal">	
	...
	<div class="form-group required" id="meta-h1-custom">	
	  <?php foreach ($languages as $language) { ?>
		...
		<div class="col-sm-10">
		  <div class="input-group"><span class="input-group-addon"><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /></span><input type="text" name="product_description[<?php echo $language['language_id']; ?>][meta_h1]" value="" placeholder="<?php echo $entry_meta_h1; ?>" class="form-control" /></div><br />
		</div>
	  <?php } ?>
	</div>
...
//--></script>
<script type="text/javascript"><!--
$('#form-mass-product-meta-h1 input[id=\'meta_tile_generator\']').on('change', function() {
	if ($('#form-mass-product-meta-h1 input[id=\'meta_tile_generator\']:checked').val()) {
