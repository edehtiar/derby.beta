<div class="modal-header">
  ...
  <h4 class="modal-title"><?php echo $manufacturer_name; ?> - <?php echo $text_store; ?></h4>
</div>
<div class="modal-body">
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-store-manufacturer" class="form-horizontal">
...
<div class="modal-footer">
  <div class="text-center"><a data-form="form-store-manufacturer" class="button-save btn btn-info btn-lg"><?php echo $button_save; ?></a></div>
</div>
...
$(".button-save").on('click', function(){
	$.ajax({
		...
		success: function(json) {
			if (json['error']) {
				$('#form-store-manufacturer').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '</div>');
				
				$('.alert-danger').delay(1500).fadeOut(500);
			}
									
			if (json['success']) {
				$('#form-store-manufacturer').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '</div>');				
