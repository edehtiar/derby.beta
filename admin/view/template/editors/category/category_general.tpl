<div class="modal-header">
  ...
  <h4 class="modal-title"><?php echo $category_name; ?> - <?php echo $text_general; ?></h4>
</div>
<div class="modal-body" id="body-category-general">
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-category-general" class="form-horizontal">	
...
<div class="modal-footer">
  <div class="text-center"><a data-form="form-category-general" class="button-save btn btn-info btn-lg"><?php echo $button_save; ?></a></div>
</div>
<script type="text/javascript"><!--
$(".button-save").on('click', function(){		
	<?php foreach ($languages as $language) { ?>
	var content = $('textarea[name="category_description[<?php echo $language['language_id']; ?>][description]"]').html($('#input-description<?php echo $language['language_id']; ?>').code());
	<?php } ?>	
	$.ajax({
		...
		success: function(json) {
			if (json['error']) {
				$('#form-category-general').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '</div>');
				
				setTimeout(function(){
					$('#body-category-general .alert-danger').addClass('hide');
				}, 1500)	
			}
								
			if (json['success']) {				
				$('#form-category-general').before('<div class="alert alert-success"><i class="fa fa-exclamation-circle"></i> ' + json['success'] + '</div>');
