  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right"><a href="<?php echo $add; ?>" data-toggle="tooltip" title="<?php echo $button_add; ?>" class="btn btn-primary"><i class="fa fa-plus"></i></a>
        <button type="button" data-toggle="tooltip" title="<?php echo $button_delete; ?>" class="btn btn-danger" onclick="confirm('<?php echo $text_confirm; ?>') ? $('#form-reviews').submit() : false;"><i class="fa fa-trash-o"></i></button>
      </div>
          ...
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($review['oct_sreview_review_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $review['oct_sreview_review_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $review['oct_sreview_review_id']; ?>" />
                    <?php } ?></td>
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=octemplates/sreview_reviews&token=<?php echo $token; ?>';
...
$('input[name=\'filter_author\']').autocomplete({
  'source': function(request, response) {
    $.ajax({
      url: 'index.php?route=octemplates/sreview_reviews/autocomplete&token=<?php echo $token; ?>&filter_author=' +  encodeURIComponent(request),
      dataType: 'json',
      success: function(json) {
        response($.map(json, function(item) {
          return {
            label: item['author'],
            value: item['oct_sreview_review_id']
          }
        }));
      }
    });
