...
class ModelModulePopupSubscribe extends Model {
	...
	public function getSubscribes($data=array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "popup_subscribe ";
	...
	public function getTotalSubscribes() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "popup_subscribe");
	...
	public function exportCSV() {
		...
		$query = "SELECT email AS `Email`, approved AS `Approved`, ip AS `IP`, date_added AS `Date Added` FROM " . DB_PREFIX . "popup_subscribe WHERE approved = '1'";
	...
	public function getSubscribe($subscribe_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "popup_subscribe WHERE subscribe_id = '" . (int)$subscribe_id . "'");
	...
	public function approve($subscribe_id) {
		...
		if ($subscribe_info) {
			...
			$message .= sprintf($this->language->get('text_unsubscribe_services'), HTTP_CATALOG . 'index.php?route=account/module_subscribe&unsubscribe=' . (int)$subscribe_id . '&hash='.$subscribe_info['hash']) . "\n\n";
	...
	public function makeDB() {
		$sql  = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "popup_subscribe` ( ";
	...
	public function removeDB() {
		$sql  = "DROP TABLE IF EXISTS `" . DB_PREFIX . "popup_subscribe`;";
