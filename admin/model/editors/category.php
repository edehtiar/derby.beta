<?php
class ModelEditorsCategory extends Model {
	public function addCategory($data) {
		...
		$this->new_category_id = $category_id;
		...
		if (isset($this->request->post['auto_keyword_category'])) {
			if ($data['keyword']) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			} else {
				$this->load->model('tool/seo_manager');
				if ($categoryName) {
					$this->model_tool_seo_manager->generateCategory($category_id, $categoryName, $this->config->get('maxy_seo_categories_template_url_alias'), $this->config->get('config_language'));
				}
			}
		} else {
			if ($data['keyword']) {
	...
	public function editCategory($category_id, $data) {
		...
		if ($this->request->post['auto_keyword_category']) {
			if ($data['keyword']) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
			} else {
				$this->load->model('tool/seo_manager');
	...
	public function copyCategory($category_id) {
		$this->event->trigger('pre.admin.category.copy', $category_id);
		...
		$this->event->trigger('post.admin.category.copy', $category_id);
	}
