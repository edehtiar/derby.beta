	...
	public function index($setting) {
                            ...
                            if($complect_data['products_data']){
                                ...
                                foreach ($complect_data['products_data'] as $product_info) {
                                    ...
                                    if ($product_info) {
                                            ...
                                            $data['complects'][$complect_id]['info']['total_complect_price_text'] = $this->currency->format($this->tax->calculate($complect_data['total_complect_price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
                                            $data['complects'][$complect_id]['info']['total_complect_old_price_text'] = $this->currency->format($this->tax->calculate($total_complect_price_text_old_price[$complect_id], $product_info['tax_class_id'], $this->config->get('config_tax')));
                                    }
                                }
                            }
			}
		...
		if (isset($data['complects']) && $data['complects']) {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/discountsales.tpl')) {
				...
				return $this->load->view('default/template/module/discountsales.tpl', $data);
			}
		}
	}
}